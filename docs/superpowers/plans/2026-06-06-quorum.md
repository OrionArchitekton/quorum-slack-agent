...
- [ ] **Step 1: Resolve AI SDK MCP export name**

Run in a scratch dir: `npm view @ai-sdk/mcp version` and `npm view ai version`. Determine whether the MCP client is `createMCPClient` (from `@ai-sdk/mcp`) or `experimental_createMCPClient` (from `ai`). Record the exact import in `docs/SPIKE-mcp.md`.
...
```json
{
  ...
  "dependencies": {
    "@quorum/shared": "workspace:*",
    "@quorum/slack": "workspace:*",
    "@workflow/ai": "PIN_COMPATIBLE_WITH_4.3.1",
    "ai": "PIN_FROM_TASK_0",
    "workflow": "4.3.1",
    "zod": "^3.23.0"
  }
...
- [ ] **Step 3: `model.ts`** (gateway model id string works with AI SDK; fake mode handled in steps)

```ts
import { cfg } from "./env.js";
export function modelId(kind: "draft" | "classify"): string {
  return kind === "draft" ? cfg.model : cfg.classifierModel;
}
```
...
export async function classifyDecisionStep(text: string): Promise<Classification> {
  ...
  const { object } = await generateObject({
    model: modelId("classify"), schema: Out,
...
export async function draftRecordStep(p: {
  ...
  if (cfg.fakeLlm) {
    ...
    const { object } = await generateObject({
      model: modelId("draft"), schema: Draft,
...
**Files:** Create `workflows/src/steps/fetchThread.ts`, `workflows/src/steps/postCard.ts`. (Thin Slack Web API wrappers; covered by integration tests, not unit.)
...
// Mock all network steps so the workflow runs offline.
vi.mock("./steps/fetchThread.js", () => ({ fetchThreadStep: async () => ({
  text: "We will use Postgres. Final.", participants: ["U1"], permalink: "https://x/p1" }) }));
vi.mock("./steps/postCard.js", () => ({ postApprovalCardStep: async () => ({ ts: "111.1" }),
  updateMessageStep: async () => {} }));
vi.mock("./steps/file.js", () => ({ fileRecordStep: async () => ({ logPermalink: "https://x/log" }) }));

import { captureWorkflow } from "./capture.js";

it("captures, waits for approval, and files on approve", async () => {
...
Expected: PASS (approve path). Add a second test for the expiry path using `getRun(run.runId).wakeUp(...)` per `@workflow/vitest` sleep helpers.
...
let seq = Date.now() % 10000; // demo-simple monotonic id source
