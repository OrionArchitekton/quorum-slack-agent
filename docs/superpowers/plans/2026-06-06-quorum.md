...
quorum/
  ...
  workflows/
    ...
    src/
      env.ts                           # config + FAKE_LLM gate
      model.ts                         # AI SDK model resolver (gateway/provider/fake)
...
npm view @ai-sdk/anthropic version    # real-LLM provider
...
**Files:** Create `workflows/package.json`, `workflows/src/env.ts`, `workflows/src/model.ts`.
...
```json
{
  ...
  "dependencies": {
    "@quorum/shared": "workspace:*",
    "@quorum/slack": "workspace:*",
    "@ai-sdk/anthropic": "PIN_FROM_TASK_0",
...
export const cfg = {
  ...
  model: () => process.env["QUORUM_MODEL"] ?? "claude-sonnet-4-5",
  classifierModel: () => process.env["QUORUM_CLASSIFIER_MODEL"] ?? "claude-haiku-4-5",
...
- [ ] **Step 3: `model.ts`** — return a real AI SDK model object (review fix: proctor passes a model object, not a bare string; non-fake path needs an actual provider)

Add `@ai-sdk/anthropic` to `workflows/package.json` dependencies (version from Task 0). Then:
...
export function model(kind: "draft" | "classify"): LanguageModel {
  return anthropic(kind === "draft" ? cfg.model() : cfg.classifierModel());
}
```

> If you prefer the Vercel AI Gateway (string ids like `"anthropic/claude-sonnet-4.5"`), set `AI_GATEWAY_API_KEY` and pass the string directly to `generateObject`/`DurableAgent` instead — but then document the gateway env requirement. Default path = `@ai-sdk/anthropic` (proven shape).
...
export async function classifyDecisionStep(text: string): Promise<Classification> {
  ...
  const { object } = await generateObject({
    model: model("classify"), schema: Out,
...
export async function draftRecordStep(p: {
  ...
  if (cfg.fakeLlm()) {
    ...
    const { object } = await generateObject({
      model: model("draft"), schema: Draft,
