...
**Tech Stack:** pnpm monorepo (mirrors the `proctor` repo), `workflow@4.3.1` + `@workflow/{ai,rollup,vitest,world-local}`, Next.js 15.5 + `@slack/bolt` + `@vercel/slack-bolt`, AI SDK (`ai`) + `@ai-sdk/mcp` + `@modelcontextprotocol/sdk`, Slack RTS (`assistant.search.context`), `zod`, `vitest` + `tsx`, TypeScript 5.6.
...
Run: `cp ../proctor/vitest.integration.config.ts ./vitest.integration.config.ts && cp ../proctor/vitest.workflow-*.js ./` then edit any `@proctor`/path refs.
